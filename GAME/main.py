...
# Background
try:
    background = pygame.image.load('GAME/game.png')
except pygame.error as e:
    print(f"Error loading image: {e}")


# Sound
mixer.music.load("GAME/explosion.wav")
mixer.music.play(-1)
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_SPACE:
                if bullet_state == "ready":
                    bulletSound = mixer.Sound("GAME/laser.wav")
                    bulletSound.play()
                    bulletX = playerX  # Set the bullet's position to the player's position
